...
use rapier::parry::transformation::{vhacd::VHACDParameters, voxelization::FillMode};
...
pub enum CollisionShape {
    ...
    /// A compound collision shape made up of multiple convex shapes that have been automatically
    /// computed from the given triangle mesh ( in 3D ) or polyline ( in 2D )
    ///
    /// A convex decomposition will take a potentially non-convex shape and break it into convex
    /// parts that can be accurately used for collision detection. This strategy is much perferred
    /// over using triangle meshes, because triangle meshes are considered hollow which can cause
    /// issues with objects going through each-other.
    ConvexDecomposition {
        /// The vertices of the shape
        vertices: Vec<Vec3>,

        /// The vertice indices that are connected to form triangles between the vertices
        #[cfg(dim3)]
        indices: Vec<[u32; 3]>,

        /// The vertice indices that are connected to form triangles edges between the vertices
        #[cfg(dim2)]
        indices: Vec<[u32; 2]>,

        /// An optional border radius that will be used to round the corners of the convex
        /// decomposition
        ///
        /// This radius refers to how much to _add_ to the existing size of the shape, creating an
        /// extra buffer around the equivalent un-rounded shape.
        border_radius: Option<f32>,

        /// Optional parameters used to customize the construction of the shape
...
// Copied from parry:
// https://docs.rs/parry2d/0.5.1/src/parry2d/transformation/vhacd/parameters.rs.html#7-52
//
// Waiting on PR: https://github.com/dimforge/parry/pull/40
//
/// Parameters controlling convex decomposition.
///
...
pub struct ConvexDecompositionParameters {
    ...
    /// voxelized.
    ///
    /// Default: `ConvexDecompositionFillMode::FloodFill { detect_cavities: false, detect_self_intersections: false }`
    pub fill_mode: ConvexDecompositionFillMode,
...
impl Default for ConvexDecompositionParameters {
    fn default() -> Self {
        Self {
            ...
            fill_mode: ConvexDecompositionFillMode::FloodFill {
...
impl Into<VHACDParameters> for ConvexDecompositionParameters {
    fn into(self) -> VHACDParameters {
        VHACDParameters {
...
// Copied from parry:
// https://docs.rs/parry2d/0.5.1/src/parry2d/transformation/voxelization/voxelized_volume.rs.html.
//
// Waiting on PR: https://github.com/dimforge/parry/pull/40
//
...
pub enum ConvexDecompositionFillMode {
...
impl Into<FillMode> for ConvexDecompositionFillMode {
    fn into(self) -> FillMode {
        match self {
            ConvexDecompositionFillMode::SurfaceOnly => FillMode::SurfaceOnly,
            ConvexDecompositionFillMode::FloodFill {
                detect_cavities,
